...
export default function CheckoutPage() {
  ...
  // Calculate subtotal with discounts applied (sum of discounted item prices)
  const subtotal = cart.reduce((total, item) => {
    const discountedPrice = getDiscountedPrice(item.price, discount);
    return total + discountedPrice * item.quantity;
  }, 0);

  // Estimate shipping logic based on cart total (same as cart page)
  ...
  // Calculate total price (subtotal - discount + shipping)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const orderData: OrderData = {
      ...
      items: cart.map((item) => ({
        ...
        price: getDiscountedPrice(item.price, discount), // Use discounted price
      })),
    ...
    try {
      ...
      Swal.fire({
        ...
        html: `<div>
          ...
          <p>Total Amount: PKR ${total.toLocaleString()}</p>
            ...
            <div className="space-y-4">
              ...
              {cart.map((item) => (
                <div key={item._id} className="flex justify-between items-center">
                  ...
                  <span className="text-lg">PKR {(getDiscountedPrice(item.price, discount) * item.quantity).toLocaleString()}</span>
                </div>
              ))}
              ...
              <div className="flex justify-between text-lg">
                <span>Shipping</span>
                <span>{shipping === 0 ? "Free Shipping" : `PKR ${shipping.toLocaleString()}`}</span>
              </div>
